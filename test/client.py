...
def node_differences(a, b, root):
    ...
    if isinstance(a, dict) and isinstance(b, dict):
        ...
        for v in a:
            if v not in b:
                errors.append(
                    'Value %s missing from b at %s' % (v[0:32], root)
                )
        for v in b:
            if v not in a:
                errors.append(
                    'Value %s missing from a at %s' % (v[0:32], root)
                )
    else:
        if a != b:
            ...
            if len(errors) > 0:
                ...
                raise Exception(
                    'Invalid request\n%s\n%s\n. Errors: %s\n' % (
                        ...
                        json.dumps(test_data['request'], indent=2), errors
                    )
                )
...
class ClientTest(unittest.TestCase):
    def testConstruction(self):
        ...
        self.assertEqual(
            tc.url, 'http://localhost:{}/transmission/rpc'.format(
                transmission_rpc.constants.DEFAULT_PORT
            )
        )
        tc = createClient(
            '127.0.0.1',
        ...
        self.assertEqual(tc.url, 'http://127.0.0.1:7000/transmission/rpc')
        tc = createClient(
            '127.0.0.1', 7000, user='user', test_name='construction'
        )
        self.assertEqual(tc.url, 'http://127.0.0.1:7000/transmission/rpc')
        tc = createClient(
    ...
    def testTimeoutProperty(self):
        tc = createClient(
            '127.0.0.1', 12345, timeout=10, test_name='construction'
        )
        self.assertEqual(tc.timeout, 10.0)
        tc.timeout = 0.1
        self.assertEqual(tc.timeout, 0.1)
        ...
        del tc.timeout
        self.assertEqual(
            tc.timeout, transmission_rpc.constants.DEFAULT_TIMEOUT
        )
        tc.timeout = '100.1'
    ...
    def testAddTorrent(self):
        ...
        self.assertEqual(
            r.hashString, 'a21c45469c565f3fb9595e4e9707e6e9d45abca6'
        )
    ...
    def testGetTorrents(self):
        ...
        for torrent in r:
            if torrent.id == 2:
                ...
                self.assertEqual(
                    torrent.name, 'ubuntu-10.04-alternate-i386.iso'
                )
                self.assertEqual(
                    torrent.hashString,
